...
void solve(){
    ...
    for (int i = 0; i < size; ++i) {
        cin >> numbers[i];
        if( !train[ numbers[i] ].isTake )
        {
            train[ numbers[i] ].isTake   = true;
            train[ numbers[i] ].firstPos = i;
            train[ numbers[i] ].secPos   = i;
        }
        else
        {
            train[ numbers[i] ].secPos = i;
    ...
    while( q-- ){
        ll a , b ;
        cin >> a >> b;
        if( (!train[a].isTake) || (!train[b].isTake) )
        {
            No<<ln;
        }
        else if( train[a].firstPos <= train[b].secPos )
        {
            Yes<<ln;
        }
        else
