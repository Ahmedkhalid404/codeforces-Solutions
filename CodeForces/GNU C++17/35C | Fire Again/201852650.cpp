...
void bfs(ll n , ll m )
{
    ...
    for (int i = 0; i < sz; ++i) {
        q.push( src[i] );
        isVisited[ src[i].first ][src[i].second] = true;
    }
    while( !q.empty() )
    {
        ll size = q.size();
        while( size-- )
        {
            pl front = q.front();
            q.pop();
            if( !isVisited[ front.first ][front.second] )
            {
            ...
            for( auto move : moves )
            {
                ll x = front.first + move.first;
                ll y = front.second + move.second;
                if( x > n || x < 1 )
                    continue;
                if( y > m || y < 1 )
                    continue;
    ...
    cout<<lvls.first<<" "<<lvls.second<<ln;
}
...
//    for(auto move : moves)
//    {
//        cout<<coordinate.front().first + move.first<<" "<<coordinate.front().second + move.second<<ln;
//    }
}
