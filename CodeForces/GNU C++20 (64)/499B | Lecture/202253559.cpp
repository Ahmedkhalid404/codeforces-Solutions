...
auto There_are_10_types_of_people_in_the_world_those_who_understand_binary_and_those_who_do_not /* :) */() {
    ll size , e;
    cin >> size >> e;
    map< string , pair<string,bool> >dictionary ;
    for (int i = 0; i < e; ++i) {
        string s1 , s2;
        cin >> s1 >> s2;
        dictionary[ s1 ].first = s2;
        dictionary[ s1 ].second = true;
        dictionary[ s2 ].first = s1;
        dictionary[ s2 ].second = false;
    }
    while( size -- )
    {
        string s;
        cin >> s;
        if( s.size() < dictionary[ s ].first.size() )
        {
            cout<<s<<" ";
        }
        else if( s.size() > dictionary[ s ].first.size() )
        {
            cout<<dictionary[s].first<<" ";
        }
        else
        {
            if( dictionary[ s ].second )
                cout<<s<<" ";
            else
                cout<<dictionary[s].first<<" ";
        }
