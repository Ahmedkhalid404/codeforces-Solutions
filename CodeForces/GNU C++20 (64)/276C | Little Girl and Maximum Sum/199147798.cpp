...
void solve(){
    ll n , q;
    cin >> n >> q;
    vector<ll>numbers(n + 1,-1),temp(n + 1);
    vector<ll>prefix( n + 5 );
    vector<TwoNumbers>query(q);
    for (int i = 1; i <= n; ++i) {
        cin >> temp[i];
    }
    ll cnt = 0;
    ...
    vector< pair<ll,ll> >toSort(n+1,{-1,-1});
    for (int i = 1; i <= n; ++i) {
        toSort[i] = { i , prefix[i] };
    }
    sort( temp.begin() + 1 , temp.end(),[](ll x, ll y){
        return x > y;
    });
    sort( toSort.begin() + 1 ,toSort.end(),[](pl x, pl y){
        return x.second > y.second;
    } );
    for (int i = 1; i <= n; ++i) {
        numbers[ toSort[i].first ] = temp[ i ];
    }
    vector<ll>realPrefix(n+1);
    sum = 0;
    for (int i = 1; i <= n; ++i) {
        sum += numbers[i];
        realPrefix[i] = sum;
    ...
    for (int i = 0; i < q; ++i) {
        ...
        l = query[i].l;
        r = query[i].r;
        sum = sum + ( realPrefix[r] - realPrefix[l - 1] );
    }
    cout<<sum;
}
