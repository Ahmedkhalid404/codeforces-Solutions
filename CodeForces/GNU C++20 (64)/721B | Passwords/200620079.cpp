...
void solve() {
    ll size , k;
    cin >> size >> k;
    vector<string>passwordsBest(size),passwordsWorst(size);
    for (int i = 0; i < size; ++i) {
        cin >> passwordsBest[i];
        passwordsWorst[i] = passwordsBest[i];
    }
    string  valid;
    cin >> valid;
    sort( all(passwordsBest) , [valid](string s1 , string s2){
        if( s1.size() == s2.size() )
        {
            if( s1 == valid )
                return true;
    ...
    sort( all(passwordsWorst) , [valid](string s1 , string s2){
        if( s1.size() == s2.size() )
        {
            if( s1 == valid )
                return false;
    ...
    for (int i = 0; i < size; ++i) {
        ...
        cnt++;
        resWorst++;
        if( passwordsWorst[i] == valid )
            break;
    }
    cout<<resBest<<" "<<resWorst;
}
