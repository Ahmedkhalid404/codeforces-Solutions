...
int main() {
    ...
    while(tt--)
    {
        int size;
        cin >> size;
        vector< vector<int> >numbers( 2 , vector<int>(size) );
        solve sum;
        for (int i = 0; i < 2; ++i) {
            for (int j = 0; j < size; ++j) {
                ...
                if( !i  && j )
                {
                    sum.sum1 += numbers[i][j];
                }
                else if( i && j != size - 1 )
                {
                    sum.sum2 += numbers[i][j];
                }
            }
        }
        if( size == 1 )
        ...
        ll mn = (ll)min( sum.sum1 , sum.sum2 );
        sum.sum3 = sum.sum1 - numbers[0][1];
        sum.sum4 = numbers[1][0];
        for (int i = 1,j = 2; i < size - 1; ++i,j++) {
            mn = min ( mn , (ll)max( sum.sum3 , sum.sum4 ) );
            sum.sum4 += numbers[1][i];
            sum.sum3 -= numbers[0][j];
        }
        cout<<mn<<ln;
    }
    //********** code ***************
