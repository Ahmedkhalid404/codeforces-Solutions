...
int idxMaxEl(vll numbers , ll st , ll ed)
{
    pl mx = { numbers[st] , st };
    for (int i = st + 1; i <= ed; ++i) {
        if( numbers[i] > mx.first )
        {
            mx.first = numbers[i];
            mx.second = i;
        }
...
void solve(vll numbers , ll depth , ll st , ll ed , map<ll,ll>&result)
{
    if( ( st < 0 ) || ( ed >= numbers.size() ) || ( st > ed ) )
        return ;
    int idx = idxMaxEl(numbers , st , ed);
    result[ numbers[ idx ] ] = depth;
    //left
    solve( numbers ,  depth + 1,  st ,  idx - 1 ,result);
    //right
    solve( numbers ,  depth + 1,  idx + 1 ,  ed ,result);
}
...
int main() {
    ...
    while(tt--){
        ...
        map<ll,ll>result;
        solve(numbers , 0 , 0 , size - 1 , result);
