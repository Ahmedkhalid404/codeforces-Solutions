...
#define sum(x)                  (long long)(accumulate(all(x),0))
...
vector< vector< ll > >train;
bool isVisited[ N ];
ll cost[ N ];
struct comp{
    bool operator() (pair< pair<ll,ll> , bool >l,pair< pair<ll,ll> , bool >r)
    {
        return (l.first.second > r.first.second);
    }
};
priority_queue< pair< pair<ll,ll> , bool >, deque<pair< pair<ll,ll> , bool >> , comp>pq;
ll result ;
void dijkstra(ll src)
{
    ...
    cost[ src ] = 0;
    pq.push({{src,cost[src]},false});
    while( !pq.empty() )
    {
        ll front = pq.top().first.first;
        ll cst = pq.top().first.second;
        bool isTrain = pq.top().second;
        pq.pop();
 
        if(cost[front] <= cst && isTrain  )
        {
            result++;
            continue;
        }
        else if(cost[ front ] > cst && isTrain )
        {
            cost[ front ] = cst;
        }
        if( isVisited[ front ] )
        ...
        for (int i = 0; i < graph[ front ].size(); ++i) {
            ll u = graph[ front ][i].first;
            ll edgeCost = graph[ front ][i].second;
            if( cst + edgeCost < cost[ u ])
            {
                if( cost[ u ] )
                cost[ u ] = cst + edgeCost;
                pq.push( {{u, cost[u]},false} );
            }
        }
    }
}
