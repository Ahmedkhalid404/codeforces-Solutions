...
enum connection{disconnect , connect};
 
void solve(){
    ll size ;
    cin >> size;
    vector< vector< connection > >graph(size,vector< connection >(size ,disconnect));
    for (int i = 0; i < size; ++i) {
        ll plane;
        cin >> plane;
        plane--;
        graph[i][plane] = connect;
    }
    for (int i = 0; i < size; ++i) {
        for (int j = 0; j < size; ++j) {
            if( i == j )
                continue;
            if( graph[i][j] == connect )
            {
                for (int k = 0; k < size; ++k) {
                    if( k == i || k == j)
                        continue;
                    if( graph[j][k] == connect )
                    {
                        if( graph[k][i] == connect )
                        {
                            Yes;
                            return;
                        }
