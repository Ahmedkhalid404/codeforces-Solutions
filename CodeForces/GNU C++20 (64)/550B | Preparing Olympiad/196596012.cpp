...
bool isPrime(ll n);
//**prototype**//
void solve(ll n , ll l , ll r , ll x , vll numbers , set< vector< pair<ll,ll> > > &preResult ,vector< pair<ll,ll> > result, ll sum , ll idx){
    if( sum > r )
        return ;
    if( ( result.size() >= 2 ) && ( sum >= l )  )
    {
        if( (max_element(result.begin(), result.end())->first - min_element(result.begin(), result.end())->first ) >= x )
        {
            Sort(result);
            preResult.insert(result);
        }
    }
    if( idx == numbers.size() )
        return ;
    solve( n , l , r , x  , numbers ,preResult, result , sum , idx + 1 );
    result.push_back( {numbers[idx] , idx} );
    solve( n , l , r , x  , numbers , preResult,result , sum + numbers[idx] , idx + 1 );
}
...
int main() {
    ...
    set< vector< pair<ll,ll> > >result;
    solve( n , l , r , x  , numbers ,result, vector< pair<ll,ll> > {} , 0 , 0 );
    cout<<result.size();
    //********** code ***************
    return 0 ;
}
