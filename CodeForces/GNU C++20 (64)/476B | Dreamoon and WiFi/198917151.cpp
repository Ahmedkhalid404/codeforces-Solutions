...
bool isPrime(ll n);
ll setBit1(ll num,ll idx);
ll setBit0(ll num ,ll idx);
ll flipBit(ll num,ll idx);
bool getBit(ll num,ll idx);
//**prototype**//
 
void SolveRec(map<char,ll>realFreq,map<char,ll>checkFreq,ll &all,ll &right){
    if( !checkFreq['?'] )
    {
        if( realFreq['-'] == checkFreq['-']  )
        {
            right++;
        }
        all++;
    ...
    checkFreq['?']--;
    checkFreq['+']++;
    SolveRec(realFreq,checkFreq,all,right);
    checkFreq['+']--;
    checkFreq['-']++;
    SolveRec(realFreq,checkFreq,all,right);
}
...
void solve(){
    string s1 , s2;
    cin >> s1 >> s2;
    map<char,ll>realFreq,checkFreq;
...
ll fPow(ll base,ll pow)
{
    if(! ( pow %mod ) ) return 1;
    cout<<base<<" "<<pow<<ln;
    ll result = fPow( ( base % mod ) ,(pow % mod ) / 2 ) % mod;
 
    return ( ( pow % mod ) & 1 ? ( (result % mod) * (result % mod) * (base % mod) ) % mod : ( (result % mod) * (result % mod) ) % mod );
}
ll setBit1(ll num,ll idx)
{
    return ( num | (1 << idx) );
