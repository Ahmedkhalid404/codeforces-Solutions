...
void drawFirstLevel(ll n,ll &space,ll MaxNumInRow = 0)
{
    bool flag = true;
    for (int i = 0; i <= n; ++i) {
        ...
        ll num = 0;
        for (int j = 0; j < ( 2 * MaxNumInRow + 1 ) ; ++j) {
            if( j == 2 * MaxNumInRow )
                cout<<num;
            else
                cout<<num<<" ";
...
void drawSecLevel(ll n ,ll space ,ll MaxNumInRow)
{
    bool flag = true;
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < space; ++j) {
...
void draw(ll n){
    ll space = n*2;
    drawFirstLevel(n,space);
    drawSecLevel(n,space,n-1);
}
