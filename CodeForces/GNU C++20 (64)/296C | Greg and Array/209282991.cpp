...
void solve() {
    ...
    vector< STOperatoin > operation( op + 5 );
    for (int i = 1; i <= op; ++i) {
        cin >> operation[i].l >> operation[i].r >> operation[i].x;
    }
    vector<ll> freqOp ( op + 5 );
    for (int i = 0; i < q; ++i) {
        ll l , r;
        cin >> l >> r;
        freqOp[ l ]++;
    ...
    vll res( size + 5 );
    for (int i = 1; i <= op; ++i) {
        res[ operation[i].l ] = res[ operation[i].l ] + ( freqOp[i] * operation[i].x );
        res[ operation[i].r + 1 ] = res[ operation[i].r + 1 ] - ( freqOp[i] * operation[i].x );
    }
    sum = 0;
    for (int i = 1; i <= size; ++i) {
        sum += res[i];
        res[i] = sum;
        cout<<res[i] + numbers[i]<<" ";
    }
}
